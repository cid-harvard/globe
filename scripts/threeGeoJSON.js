...
function drawThreeGeo(json, radius, shape, options) {
    ...
    /*var points=segments.vertices;
    var shape = new THREE.Shape();
    shape.moveTo(points[0].x,points[0].y);
    for (var i = 1; i <points.length-1; i+=3) {
        if(points[i].x>-1000 && points[i].x<-300 &&points[i].y>150)shape.lineTo(points[i].x,points[i].y);
    };
    //shape.lineTo(points[0].x,points[0].y);
    var geometry = new THREE.ShapeGeometry( shape );

    var mesh = new THREE.Mesh( geometry, new THREE.MeshPhongMaterial( { color: "white", side: THREE.DoubleSide } ) );
    mesh.position.set(700,-300,0);
...
function drawLine(x_values, y_values, z_values, options) {
    ...
    if(x_values[0]>-1000 && x_values[0]<-300 && y_values[0]>150)lines.add(line);
